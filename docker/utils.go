...
func extractSubscriptions(service string, labels map[string]string) []models.Subscription {
	...
	subscriptionMap := make(map[string]*models.Subscription)
	...
	for key, value := range labels {
		...
		if _, ok := subscriptionMap[name]; !ok {
			subscriptionMap[name] = &models.Subscription{
				Service: service,
				Name:    name,
			}
		}
...
func extractEventType(action string) EventType {
	switch action {
	case subscribeEventName:
		return EVENT_TYPE_SUBSCRIBE
	case unsubscribeEventName:
